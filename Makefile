...
cedar: dist/cedar/gdal-1.10.1-1.tar.gz dist/cedar/proj-4.8.0-1.tar.gz dist/cedar/geos-3.4.2.tar.gz

cedar-14: dist/cedar-14/gdal-1.10.1-1.tar.gz dist/cedar-14/proj-4.8.0-1.tar.gz

dist/cedar/gdal-1.10.1-1.tar.gz: gdal-cedar
	docker cp $<:/tmp/gdal-cedar.tar.gz .
	mkdir -p $$(dirname $@)
	mv gdal-cedar.tar.gz $@

dist/cedar/geos-3.4.2.tar.gz: gdal-cedar
	docker cp $<:/tmp/geos-cedar.tar.gz .
	mkdir -p $$(dirname $@)
	mv geos-cedar.tar.gz $@
...
dist/cedar-14/gdal-1.10.1-1.tar.gz: gdal-cedar-14
...
src/gdal.tar.gz:
	mkdir -p $$(dirname $@)
	curl -sL http://download.osgeo.org/gdal/1.10.1/gdal-1.10.1.tar.gz -o $@
